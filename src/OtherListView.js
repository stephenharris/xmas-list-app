            ...
            {items.map((item) => {
              return (<li key={item.id} id={item.id}>
                <span>
                  ...
                  <label className={classNames({'item-bought-by-you': item.boughtBy === "you", 'item-bought-by-someoneelse': item.boughtBy === "someoneelse", 'mark': true})}>
                    <input type="checkbox" disabled={item.boughtBy === 'someoneelse'} checked={item.boughtBy} value={item.id} onChange={(event) => onClickToggleMark(event, item)}/>
